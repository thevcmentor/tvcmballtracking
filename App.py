...
# Lazy-load YOLO (speeds up boot; avoids timeouts)
yolo_model = None
def get_model():
    global yolo_model
    if yolo_model is None:
        from ultralytics import YOLO
        yolo_model = YOLO("yolov8n.pt")  # swap with your weights later
    return yolo_model
...
def analyze_video(request: VideoRequest):
    ...
    try:
        ...
        model = get_model()
        traj = []
        for _ in range(min(frames, 300)):  # cap frames for demo
            ok, frame = cap.read()
            if not ok: break
        ...
        cap.release()
        # very rough speed estimate (tune later via calibration)
        speed_kph = 0.0
        if len(traj) > 2:
            diffs = [((traj[i+1][0]-traj[i][0])**2 + (traj[i+1][1]-traj[i][1])**2) ** 0.5
